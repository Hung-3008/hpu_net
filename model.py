...
class ResBlock(nn.Module):
    def __init__(self, input_channels, n_channels, n_down_channels, convs_per_block, activation_fn):
        ...
        # Activation function for pre-activation
        self.activation = activation_fn()
        ...
        in_channels = input_channels
        for i in range(convs_per_block):
            out_channels = n_down_channels if i < convs_per_block - 1 else n_down_channels  # Intermediate layers stay at n_down_channels
            conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)
            layers.append(conv)
            if i < convs_per_block - 1:  # Apply activation after all but the last convolution
                layers.append(self.activation)
            in_channels = out_channels
        self.conv_layers = nn.Sequential(*layers)

        # Projection from n_down_channels to n_channels if needed
        self.proj = nn.Conv2d(n_down_channels, n_channels, kernel_size=1) if n_down_channels != n_channels else nn.Identity()
    ...
    def forward(self, x):
        ...
        # Pre-activated convolutional path
        out = self.activation(x)  # Pre-activation
        out = self.conv_layers(out)
        out = self.proj(out)  # Project to n_channels if needed
...
class HierarchicalProbUNet(nn.Module):
    ...
    def _build(self, seg, img):
        ...
        if self._cache is not None and all(torch.equal(a, b) for a, b in zip(self._cache, inputs)):
            ...
            concat_input = torch.cat([seg, img], dim=1)
